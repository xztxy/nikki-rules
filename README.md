...
- `rules/custom/proxy/`: 你的自定义代理规则，按 AI、开发、媒体、支付、成人内容和杂项拆分。
...
- `custom_proxy_media`: 媒体数据库和资源站，走 `🌐 全部节点`。
- `custom_proxy_adult`: 成人/媒体站点，走 `🌐 全部节点`。
- `custom_proxy_payments`: 海外支付和 API，走 `🌐 全部节点`。
- `custom_proxy_misc`: 其它手动代理规则，走 `🌐 全部节点`。
